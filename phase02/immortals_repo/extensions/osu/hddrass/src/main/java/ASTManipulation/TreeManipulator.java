...
public class TreeManipulator {
                ...
                for (Object b2 : t.getMembers()) {
                    ...
                    if (b.getClass().getSimpleName().toString().equals("MethodDeclaration")) {
                        ...
                        if (m.getName().toString().equals(testMethod)) {
                            return m.getBody().isPresent() ? GetHeighestDepthLevel(m.getBody().get()) : 0;
        ...
        for (TypeDeclaration t : cu.getTypes()) {
            for (Object b2 : t.getMembers()) {
                ...
                if (b.getClass().getSimpleName().toString().equals("MethodDeclaration")) {
                    MethodDeclaration m = (MethodDeclaration) b;
                    if (m.getName().toString().equals(testMethod)) {
                        leafStatements.addAll(GetAllLeafNodes(m.getBody().get()));
        ...
        for (TypeDeclaration t : cu.getTypes()) {
            for (Object b2 : t.getMembers()) {
                ...
                if (b.getClass().getSimpleName().toString().equals("MethodDeclaration")) {
                    MethodDeclaration m = (MethodDeclaration) b;
                    if (m.getName().toString().equals(testMethod)) {
                        plusone.addAll(GetAllLeafPlusOneNodes(m.getBody().get()));
    ...
    public List<Statement> GetAllLeafNodes(TryStmt tryStmt){
        ...
        BlockStmt finallyPart = tryStmt.getFinallyBlock().isPresent() ? tryStmt.getFinallyBlock().get() : null;
    ...
    private int GetHeighestDepthLevel(SwitchStmt s) {
        ...
        for (SwitchEntryStmt casestmt:  caseStmts ){
            ...
            if(casestmt == null || casestmt.getStatements() == null){
                continue;
            }
            int maxInner = 0;
            for (Statement sInner: casestmt.getStatements()) {
    ...
    private int GetHighestDepthLevel(SynchronizedStmt syncStmt){
        ...
        BlockStmt blkStmt = (BlockStmt)syncStmt.getBody();
        return GetHeighestDepthLevel(blkStmt);
    }
    ...
    private int GetHeighestDepthLevel(IfStmt ifStmt){
        ...
        try{
            ...
            if(ifStmt.getElseStmt().isPresent()  && StatementTypeQuery.isExprStmt(ifStmt.getElseStmt().get()))
                elseLength = 1;
            else{
                if(ifStmt.getElseStmt().isPresent()){
                    if(StatementTypeQuery.isIfElseStmt(ifStmt.getElseStmt().get())){
                        elseLength  = 1 + GetHeighestDepthLevel((IfStmt)ifStmt.getElseStmt().get());
                    }
                    else{
                        elseLength = GetHeighestDepthLevel((BlockStmt)ifStmt.getElseStmt().get());
                    }
        ...
        if(tryStmt.getCatchClauses() != null) {
            if(tryStmt.getCatchClauses().size() != 0){
                for (CatchClause catchClause:tryStmt.getCatchClauses()) {
                    catchlength = Integer.max(catchlength,GetHeighestDepthLevel(catchClause.getBody()));
    ...
    private boolean ContainsStatement(BlockStmt blkStmt, List<Statement> childStatements){
        boolean bIf = false;
        for (Statement s: blkStmt.getStatements()) {
    ...
    private boolean ContainsStatement(TryStmt tryStmt,Statement child){
        ...
        if(EmptyStatementChecker.isEmptyStatement(tryStmt.getTryBlock())){
            if(EmptyStatementChecker.isEmptyStatement(tryStmt.getFinallyBlock().get())){
                boolean allcatchCauseEmpty = true;
                for (CatchClause catchClause: tryStmt.getCatchClauses()) {
                    if(EmptyStatementChecker.isEmptyStatement(catchClause.getBody()))
                        continue;
                    else{
        ...
        if(!EmptyStatementChecker.isEmptyStatement(tryStmt.getFinallyBlock().get())){
            foundInFinallyClause = ContainsStatement(tryStmt.getFinallyBlock().get(),child);
        }
        for (CatchClause catchClause: tryStmt.getCatchClauses())
        {
            foundInFinallyClause = foundInCatchClauses || ContainsStatement(catchClause.getBody(),child);
    ...
    private boolean ContainsStatement(IfStmt ifstmt, Statement child){
        ...
        if(ifstmt.getElseStmt().isPresent() && StatementTypeQuery.isExprStmt(ifstmt.getElseStmt().get())){
            ...
            if(ifstmt.getElseStmt().isPresent() ){
                if(StatementTypeQuery.isIfElseStmt(ifstmt.getElseStmt().get())){
                    if(StatementTypeQuery.isIfElseStmt(ifstmt.getElseStmt().get())){
                        bElse = ContainsStatement(ifstmt.getElseStmt().get(),child);
                    }
                    else{
                        bElse = ContainsStatement((BlockStmt)ifstmt.getElseStmt().get(),child);
                    }
