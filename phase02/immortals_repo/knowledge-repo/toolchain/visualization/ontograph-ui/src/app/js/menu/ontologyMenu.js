...
module.exports = function (graph) {
	...
	var ontologyMenu = {},
		DEFAULT_JSON_NAME = "immortals_core", // This file is loaded by default
	...
	function loadOntologyFromUri(relativePath, requestedUri) {
		...
		if (cachedOntology) {
			loadOntologyFromText(cachedOntology, undefined, filename);
			setLoadingStatus(true);
			console.log("loaded from cache");//TODO
		} else {
			displayLoadingIndicators();
			
			/*
			//TODO: load the ontology model from the server instead of from a file
			console.log(relativePath);
			d3.xhr("http://localhost:8080/immortalsTestService/ping", "application/json", function (error, request) {
				if(error !== null){
					console.log("error");
			...
			*/
			...
			console.log(relativePath);
			//http://localhost:8080/immortalsVisualizationService/ttlJson?ttlDoc=data/immortals-core.ttl
			d3.xhr("http://localhost:8080/immortalsVisualizationService/ttlJson?ttlDoc="+relativePath, "application/json", function (error, request) {
				var loadingSuccessful = !error;
				var errorInfo;
