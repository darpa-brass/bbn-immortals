...
warning="
IMPORTANT: Running this file is not needed for the DAS system.

This file will create query training data files in the current directory.

If a file with the same name already exists, it will be deleted.

You can check in the resulting files to the appropriate location in the Immortals repo (e.g., castor folder). 

This script assumes the takrptbase schema in the local Immortals database is populated.

You should run this file like so:

(1) Create tmp dir in database/server dir and make sure the user postgres has read/write privs to it.
(2) Run the following from database/server/tmp: sudo -u postgres ../create_training_data.sh
