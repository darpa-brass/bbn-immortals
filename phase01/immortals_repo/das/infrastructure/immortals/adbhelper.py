...
_adb_process_start_command = (ADB_BIN, '-s', _ID_ADB_DEVICE_IDENTIFIER, 'shell', 'am', 'start', '-n', _ID_PACKAGE_ACTIVITY)
_adb_process_stop_command = (ADB_BIN, '-s', _ID_ADB_DEVICE_IDENTIFIER, 'shell', 'am', 'force-stop', _ID_PACKAGE)
_adb_grant_permission_command = (ADB_BIN, '-s', _ID_ADB_DEVICE_IDENTIFIER, 'shell', 'pm', 'grant', _ID_PACKAGE, _ID_PERMISSION)
...
_adb_logcat_flush_with_tag = (ADB_BIN, '-s', _ID_ADB_DEVICE_IDENTIFIER, 'logcat', '-v', 'raw', '-b', 'main', '-c', '-s', _ID_TAG)
...
class AdbHelper:
    ...
    def wait_for_device_ready(self):
        ...
        print 'Waiting for emulator ' + self.adb_device_identifier + ' to finish starting',
        sys.stdout.flush()
        ...
        print '\nEmulator ' + self.adb_device_identifier + ' has finished starting.'
