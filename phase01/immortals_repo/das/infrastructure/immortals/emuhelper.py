...
from utils import replace, get_formatted_string_value

_ID_EMULATOR_IDENTIFIER = '$EMULATOR_IDENTIFIER!'
_ID_PORT = '$PORT!'
_ID_EMULATOR_SDK_LEVEL = '$EMULATOR_SDK_LEVEL!'

_android_emulator_create_command = (ANDROID_BIN, 'create', 'avd', '--target', 'android-' + _ID_EMULATOR_SDK_LEVEL, '--abi', 'default/x86_64', '--device',  'Nexus 5', '--sdcard', '512M', '--name', _ID_EMULATOR_IDENTIFIER)
...
_android_emulator_start_command = (EMULATOR_BIN, '-avd', _ID_EMULATOR_IDENTIFIER, '-gpu', 'off', '-noaudio', '-port', _ID_PORT)
...
class EmuHelper:
    ...
    def create_emulator(self):
        ...
        replace(call_list, _ID_EMULATOR_SDK_LEVEL,str(configurationmanager.Configuration.deployment_environments['android_emulator'].sdk_level))
    ...
    def start_emulator(self, console_port, display_ui, sdcard_filepath):
        ...
        if display_ui:
            call_list.append('-scale')
            call_list.append('0.4')
        else:
