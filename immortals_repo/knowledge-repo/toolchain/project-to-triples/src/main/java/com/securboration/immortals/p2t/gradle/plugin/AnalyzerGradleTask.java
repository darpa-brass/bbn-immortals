...
public class AnalyzerGradleTask extends DefaultTask {
    ...
    public void analysis() {
        ...
        p.getConfigurations().getNames().stream().forEach(listName -> {
        ...
        try {
            String result = "";
            result = p2tm.gradleDataToTriples(data);
            FileWriter fw = new FileWriter(data.getCompiledProjectName() + "-output.ttl");
            fw.write(result);
            fw.flush();
            fw.close();
            ...
            if (Boolean.parseBoolean(pushContext)) {
                ...
                fw = new FileWriter("pushContextLog.txt");
