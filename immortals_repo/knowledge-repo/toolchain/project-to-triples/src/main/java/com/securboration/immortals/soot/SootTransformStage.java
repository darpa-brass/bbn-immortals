...
public class SootTransformStage {
    ...
    private Pair<File, BytecodeArtifactCoordinate> getDfuModuleDependency(GradleTaskHelper taskHelper, String dfuUUID) throws IOException, XmlPullParserException {
        ...
        Optional<File> dfuModuleResourceOption = searchForDfuModuleResource(repoPath, dfuModuleCoordinate);

        return new Pair<>(dfuModuleResourceOption.orElse(null), dfuModuleCoordinate);
    }

    private static Optional<File> searchForDfuModuleResource(String repoPath, BytecodeArtifactCoordinate dfuModuleCoordinate) throws IOException, XmlPullParserException {
        ...
        String[] groupSplits = dfuModuleCoordinate.getGroupId().split("\\.");
        ...
        repoProbeOption = Arrays.stream(Objects.requireNonNull(repoProbe.listFiles((current, name) ->
                new File(current, name).isDirectory()))).filter(dfuFile -> dfuFile.getName().equals(dfuModuleCoordinate.getArtifactId())).findAny();
        ...
        repoProbeOption = Arrays.stream(Objects.requireNonNull(repoProbe.listFiles((current, name) ->
                new File(current, name).isDirectory()))).filter(dfuFile -> dfuFile.getName().equals(dfuModuleCoordinate.getVersion())).findAny();
    ...
    public AspectConfigureSolution getChosenSolution() {
        return chosenSolution;
    }

    public void setChosenSolution(AspectConfigureSolution chosenSolution) {
