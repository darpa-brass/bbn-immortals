...
public class GradleTaskHelper {
    ...
    public Optional<String> augmentAdaptationSurfaceMethods(String wrappedClassName, String source,
        ...
        this.getClient().executeSelectQuery(getAugmentationSurfaceMethods.replace("???ASPECT???",
                ...
                , solutionSet);
        methodVisitor = constructVisitor(solutionSet);
        
        //user specified parameter configurations OR
        //default to current-best configurations
        String cipherInitBody = wrapper.getConfigurationParameters().orElseGet(this::retrieveCurrentBestConfig);
        ...
        for (FunctionalAspect functionalAspect : wrapper.getAspectsAdapted()) {
            switch (functionalAspect.getAspectId()) {
                case "cipherEncrypt":
                    newAdaptationSurface = getOutputStreamAdaptations(complexity, getAugmentationSurfaceMethods, compilationUnit);
                    break;
                case "cipherDecrypt":
                    newAdaptationSurface = getInputStreamAdaptations(complexity, getAugmentationSurfaceMethods, compilationUnit);
                    break;
            }
        }
