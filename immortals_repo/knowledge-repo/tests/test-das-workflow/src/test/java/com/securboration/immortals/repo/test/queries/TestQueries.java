...
public class TestQueries extends QueryTestBase {
    ...
    public void testSelectLocationProviders(){
        FusekiClient client = super.acquireFusekiConnection();
        AssertableSolutionSet q = new AssertableSolutionSet();
        
        final String graph = "GRAPH <http://localhost:3030/ds/data/IMMoRTALS_r2.0.0>";
        
        final String query = super.makeQuery(
                "${PREFIXES}"+
                "SELECT DISTINCT ?className ?dfu ?classPointer\n"+
                "  WHERE {\n"+
                "      ${GRAPH} {\n"+
                "        ?dfu a dfu:DfuInstance .\n"+
                "        ?dfu im:hasFunctionalityAbstraction lp_func: .\n"+
        ...
        q.assertContainsAtLeastNSolutions(new Solution(new String[]{
        ...
        for(String classPointer:q.getSolutionsForVar("classPointer")){
            getContainingJarContents(graph,classPointer);
        }
        
        System.out.println("done.");
    }
    
    private void getContainingJarContents(
            final String graph,
            final String bytecodePointer
            ){
        FusekiClient client = super.acquireFusekiConnection();
        AssertableSolutionSet q = new AssertableSolutionSet();
        ...
        final String query = super.makeQuery(
                "${PREFIXES}"+
                "SELECT DISTINCT ?jarName ?artifactId ?groupId ?version \n"+
                "  WHERE {\n"+
                "      ${GRAPH} {\n"+
                "        ?jar a bytecode:JarArtifact .\n"+
                "        ?jar im:hasJarContents ?class .\n"+
                "        ?class im:hasClassModel ?classModel .\n"+
                "        ?classModel im:hasBytecodePointer ${POINTER} .\n"+
                "        ?jar im:hasName ?jarName .\n"+
