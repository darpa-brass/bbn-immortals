...
public class ZipHelper{
    ...
    public static byte[] getZipEntry(
            final byte[] archive, 
            final String name
            ) throws IOException{
        
        final String normalTarget = name.replace("\\", "/");
        try(ZipInputStream zis = new ZipInputStream(new ByteArrayInputStream(archive))){
            while(true){
                ZipEntry entry = zis.getNextEntry();
                
                if(entry == null){
                    throw new RuntimeException("could not find " + normalTarget);
                }
                
                final String normalName = entry.getName().replace("\\","/");
                
                if(normalName.equals(normalTarget)){
                    ByteArrayOutputStream data = new ByteArrayOutputStream();
                    IOUtils.copy(zis, data);
