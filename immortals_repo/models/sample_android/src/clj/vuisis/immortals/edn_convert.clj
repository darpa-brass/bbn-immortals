(ns vuisis.immortals.edn-convert
  "Convert the deployment instance into RDF updates.
  see immortals/maven/analysis/classes/JavaToRdf.java "
  (:require
...
(defn add-resource-obj
  ...
  ([uuid-prefix zipr-loc recorder! type]
    (let [zipr-obj (zip/sexpr zipr-loc)]
      (if (:res zipr-obj)
        zipr-loc ;; if the value is already set we are good
        (let [uuid (str uuid-prefix "-" (:uuid zipr-obj))
              resource (.create-resource! recorder! uuid)]
          (.add-fact! recorder! resource RDF/type type)
          (zip/edit zipr-loc
            #(assoc-in % [:res] resource)))))))
...
(defmethod register :environment
  [loc recorder!]
  (add-resource-obj "im:artifact" loc recorder! "IMMoRTALS_resources:ExecutionPlatform"))
...
(defmethod factor :component
  ...
  (let [component (-> component-loc zip/sexpr)
      ...
      (.add-fact! recorder! focus :type (:purpose component))
      (.add-fact! recorder! focus :name (:name component))
      (.add-fact! recorder! focus :uuid (:uuid component))
      (.add-fact! recorder! frame :member focus))
  component-loc)
...
(defn add-factor-node
  ...
  (let [record (-> loc zip/sexpr)
      ...
      (.add-fact! recorder! focus :type (:purpose record))
      (.add-fact! recorder! focus :name (:name record))
      (.add-fact! recorder! focus :uuid (:uuid record))
      (.add-fact! recorder! frame :member focus))
  loc)
...
(defmethod factor :resource
  ...
  (let [resource (-> resource-loc zip/sexpr)
      ...
      (.add-fact! recorder! focus :type (:type resource))
      (.add-fact! recorder! focus :label (:action resource))
      (.add-fact! recorder! focus :uuid (:uuid resource))
      (.add-fact! recorder! focus :value (:value resource))
      (.add-fact! recorder! node :member focus))
  resource-loc)
