...
public class Reducer {
    ...
    public BlockStmt reduce(BlockStmt b, List<Statement> l){
        ...
        newBlock.setStatements(newList);
        return newBlock;
    ...
    private void ReduceTryCatchStmt(List<Statement> l, List<Statement> newList, TryStmt tryStmt){
        if(!l.contains(tryStmt)){
            ...
            NodeList<CatchClause> catchClausesToBeRetained = new NodeList<>();
            if(tryStmt.getCatchClauses() != null){
                for (CatchClause catchClause:tryStmt.getCatchClauses() ) {
                    CatchClause tempCatchClause = new CatchClause();

                    tempCatchClause.setParameter(catchClause.getParameter());
                    tempCatchClause.setBody((BlockStmt) reduce(catchClause.getBody(),l));

                    catchClausesToBeRetained.add(tempCatchClause);


                }
                tryStmt.setCatchClauses(catchClausesToBeRetained);
    ...
    private void ReduceSwitchCase(List<Statement> l, SwitchStmt s) {
        ...
        for ( SwitchEntryStmt switchEntryStmt : switchStmt.getEntries()  ) {
            NodeList<Statement> statementsToBeRetained = new NodeList<>();
            for (Statement s2: switchEntryStmt.getStatements()) {
            ...
            switchEntryStmt.setStatements(statementsToBeRetained);
    ...
    private void ReduceSynchronizedStmt(List<Statement> l, List<Statement> newList, Statement s2){
        if(!l.contains(s2)){
            ...
            if(!EmptyStatementChecker.isEmptyStatement(syncstmt.getBody())){
                syncstmt.setBody(reduce(syncstmt.getBody(),l));
            }
