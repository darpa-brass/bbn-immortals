...
IMMORTALS_ROOT = os.path.join(_SCRIPT_DIRECTORY, 'external', 'immortals_root')

if not os.path.exists(IMMORTALS_ROOT):
    IMMORTALS_ROOT = os.path.realpath(os.path.join(_SCRIPT_DIRECTORY, '..', '..', '..'))

SWRI_EXAMPLE_ROOT = os.path.join(_SCRIPT_DIRECTORY, 'external', 'challenge-problems')
if not os.path.exists(SWRI_EXAMPLE_ROOT):
...
SWRI_S6_VALIDATION_DOCS_ROOT = os.path.join(IMMORTALS_ROOT, 'phase3', 'utils', 'bbn_test_scenarios', 'Scenario_6',
                                            'swri_validation_docs')

SCENARIO_CONFIG_DIRECTORY = os.path.join(IMMORTALS_ROOT, 'phase3', 'immortals-orientdb-server', 'src', 'main',
                                         'resources')

KR_XTS_TESTER_JAR = os.path.join(IMMORTALS_ROOT, 'knowledge-repo', 'cp', 'cp3.1', 'xsd-translation-service-test',
                                 'target', 'immortals-xsd-translation-service-tester-boot.jar')
    ...
    def get_validation_document_dir(self):
        if self.initialXsdVersion == 'V0_8_19' and self.updatedXsdInputPath is not None:
            if self.scenarioType == 'Scenario6bbn':
                ...
                validation_xml_dir = os.path.join(SWRI_S6_VALIDATION_DOCS_ROOT, self.shortName, 'messages')
...
def _get_scenarios(source_files):
    ...
    for filepath in source_files:
        if not os.path.isabs(filepath):
            filepath = os.path.join(SCENARIO_CONFIG_DIRECTORY, filepath)
