...
def exec_scenario_5(env_values) -> bool:
    cmd = ['java', '-jar', 'flighttest-constraint-solver/flighttest-constraint-solver.jar']
    p = subprocess.run(cmd, env=env_values, stdout=sys.stdout, stderr=sys.stderr)
    return p.returncode == 0


def exec_scenario_6(env_values, be_exceptional: bool = True) -> bool:
...
def main():
    ...
    if args.start_persistence_server:
        ...
        for config in test_scenarios:
            ...
            if config[0] == '5':
                passed = exec_scenario_5(env_values)

            elif config[0] == '6':
                passed = exec_scenario_6(env_values)
        ...
        if args.persistence_url is None:
            _parser.print_usage()
            print(
                "IMMoRTALS Scenario 5/Scenario 6 Launcher: error: the following arguments are required: --persistence-url")
            exit(1)
        ...
        if args.scenario == '5':
            exec_scenario_5(env_values)

        elif args.scenario == '6':
            exec_scenario_6(env_values)
