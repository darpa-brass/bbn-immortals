...
public class MdlDataValidator extends DataValidator {
	...
	public MdlDataValidator(@Nullable File inputExcelFile, @Nullable File outputDrlFile, DataSourceInterface dataSource) {
		...
		this.collector = new DataCollector(dataSource, Configuration.getInstance().transformation);
	...
	public ValidationDataContainer validateConfiguration(@Nonnull ValidationScenario scenario, boolean useColor) {
		try {
			...
			switch (scenario) {
				...
				case ResultantConfiguration:
					outputFile = SOLVER_OUTPUT_FILE;
			...
			try {
				FileUtils.writeStringToFile(outputFile, Utils.difGson.toJson(dynamicData), Charset.defaultCharset());
			} catch (IOException e) {
				throw AdaptationnException.internal(e);
			}

			init();
			ValidationDataContainer results = super.validate(scenario.filter, dynamicData, scenario.configuration);
