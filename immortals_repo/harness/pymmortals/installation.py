...
# def get_true_path(name):
#     # OSError if the which command fails
#     try:
#         path = subprocess.check_output(['which', name]).replace('\n', '')
#     except OSError:
#         return None
#
#     try:
#         # OSError if the path is not a link, so it is the actual path
#         while True:
#             p = os.readlink(path)
#             path = p
#     except OSError:
...
_installation_configuration = InstallationConfiguration.from_dict(resourcemanager.load_installation_configuration())
...
def resolve_platform_configuration():
    ...
    if platform in _installation_configuration.targetInstallationPlatforms:
        return _installation_configuration.targetInstallationPlatforms[platform]
    else:
        ...
        if len(executables) > 0:
            for exe in executables:
                ...
                if source_path_folder not in exports:
                    ...
                    target_path = os.path.join(_installation_configuration.installationRoot, 'bin', executable)
...
class Installer:
    ...
    def _get_install_lines_install_new_app(self):
        ...
        if target_home is not None and os.path.exists(target_home) \
                and _installation_configuration.installationRoot in target_home \
            ...
            tmp = os.path.join(_installation_configuration.tempDirectory, tmp[tmp.rfind('/') + 1:])
    ...
    def _generate_all_commands(self):
        ...
        if self.configuration.homeExportRequired:
            if self._get_existing_home() is not None \
                ...
                installation += self._get_install_lines_use_current_app()
                initialization += self._get_init_lines_use_current_home()

            else:
                installation += self._get_install_lines_install_new_app()
                initialization += self._get_init_lines_export_new_home()

        else:
            if self._executables_exist_in_path() \
                    and self._is_installed_proper_version(home=None):
                installation += self._get_install_lines_use_current_app()
                initialization += self._get_init_lines_ignore_home()
                initialization += self._get_init_lines_use_current_executables()
                ...
                installation += self._get_install_lines_install_new_app()
                initialization += self._get_init_lines_install_new_app()

        self._generated_installation_commands = installation
        self._generated_initialization_commands = initialization
